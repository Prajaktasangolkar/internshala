...
function GameTab({ user }) {
  ...
  useEffect(() => {
    const fetchShapesData = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:5000/api/game/shapes'); // Use relative URL
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap' }}>
        {shapes.map((shape) => (
          <div
            key={shape.id}
            style={shape.type == 'triangle' ? triangle(shape.color)  : {
              width: '50px',
              height: '50px',
              margin: '10px',
